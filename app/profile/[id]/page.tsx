...
import {
  User, Phone, ShieldCheck, Settings, CheckCircle2,
  Flame, Heart, Zap, Wine, Ban, Dog, Languages, Sparkles,
  Users, Star, MessageCircle, Ruler, Baby, Briefcase, GraduationCap,
  MapPin, Instagram, Globe, DollarSign
} from "lucide-react";
...
export default function ProfilePage({ params }: { params: Promise<ProfileParams> }) {
  ...
  const isOwnProfile = isLoggedIn && (currentUser?.id === profileId || profileId === "me");
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      ...
      {/* --- HERO SECTION --- */}
      <div className="relative h-[550px] md:h-[700px] w-full bg-muted overflow-hidden">
        ...
        <div className="absolute bottom-12 left-6 md:left-12 z-20 space-y-4">
          <div className="flex items-center gap-4">
            ...
            {displayUser.verified && <CheckCircle2 className="text-blue-400 fill-blue-400" size={36} />}
          </div>
        ...
        <div className="absolute top-8 md:top-12 right-6 md:right-12 z-30 flex gap-3 md:gap-4">
          {!isOwnProfile ? (
            <>
              {/* Like Button */}
              <button className="h-14 w-14 md:h-16 md:w-16 bg-white/20 backdrop-blur-xl border border-white/30 text-white hover:text-rose-500 rounded-full flex items-center justify-center shadow-2xl hover:scale-110 hover:bg-white transition-all duration-300">
                <Heart size={28} className="md:w-[32px]" fill="none" strokeWidth={2.5} />
              </button>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-16 grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="bg-card border border-border rounded-[3rem] p-8 space-y-8 shadow-sm">
            <div>
              <h3 className="text-[10px] font-black uppercase text-primary tracking-widest italic mb-6">Profile Basics</h3>
              <div className="grid grid-cols-2 gap-4">
                <LifestyleBadge icon={<Star size={14} />} label="Zodiac" value={displayUser.zodiac} />
                <LifestyleBadge icon={<MessageCircle size={14} />} label="Chat Style" value={displayUser.communicationStyle} />
                <LifestyleBadge icon={<Ruler size={14} />} label="Height" value={displayUser.height ? `${displayUser.height} cm` : '—'} />
                <LifestyleBadge icon={<DollarSign size={14} />} label="Budget" value={displayUser.budgetRange} />
              </div>
            </div>

            <div className="pt-8 border-t border-border space-y-6">
              <InfoItem icon={<ShieldCheck size={20} />} label="Trust Level" value={`${displayUser.trustScore}% Verified`} />
              <InfoItem icon={<MapPin size={20} />} label="Lives In" value={displayUser.location || "Tamluk"} />
              <InfoItem icon={<Briefcase size={20} />} label="Working As" value={displayUser.occupation} />
              <InfoItem icon={<GraduationCap size={20} />} label="Studied At" value={displayUser.education} />
            </div>
          </div>

          <div className="bg-card border border-border rounded-[3rem] p-8">
            <h3 className="text-[10px] font-black uppercase text-primary tracking-widest italic mb-6">Lifestyle & Socials</h3>
            <div className="grid grid-cols-2 gap-4 mb-6">
              <LifestyleBadge icon={<Ban size={14} />} label="Smoke" value={displayUser.lifestyle?.smoking} />
              <LifestyleBadge icon={<Wine size={14} />} label="Drink" value={displayUser.lifestyle?.drinking} />
              <LifestyleBadge icon={<Dog size={14} />} label="Pets" value={displayUser.lifestyle?.pets} />
              <LifestyleBadge icon={<Languages size={14} />} label="Languages" value={displayUser.languages?.join(", ")} />
            </div>

            <div className="space-y-3">
              {displayUser.socialLinks?.instagram && (
                <a href={`https://instagram.com/${displayUser.socialLinks.instagram}`} target="_blank" className="flex items-center gap-4 p-4 bg-pink-500/10 rounded-2xl text-pink-500 font-bold text-xs uppercase italic transition-all hover:bg-pink-500/20">
                  <Instagram size={18} /> @{displayUser.socialLinks.instagram}
                </a>
              )}
              {displayUser.socialLinks?.website && (
                <a href={`https://${displayUser.socialLinks.website}`} target="_blank" className="flex items-center gap-4 p-4 bg-blue-500/10 rounded-2xl text-blue-500 font-bold text-xs uppercase italic transition-all hover:bg-blue-500/20">
                  <Globe size={18} /> {displayUser.socialLinks.website}
                </a>
              )}
            </div>
          </div>
        ...
        {/* --- প্রধান কন্টেন্ট (MAIN CONTENT) --- */}
        <div className="lg:col-span-8 space-y-12">
          {/* Bio Section */}
          <div className="bg-card border border-border rounded-[3.5rem] p-12 relative overflow-hidden group">
            <Sparkles className="absolute top-8 right-8 text-primary opacity-10 group-hover:opacity-30 transition-opacity" size={50} />
            <h3 className="text-[10px] font-black uppercase text-primary mb-6 tracking-[0.4em] italic">The Vibe</h3>
            <p className="text-3xl font-medium italic opacity-80 leading-snug">
              "{displayUser.bio || "Searching for an adventure partner..."}"
            </p>
          </div>

          {/* Dating Prompt Section */}
          ...
          <div className="bg-secondary/10 border border-secondary/20 rounded-[3rem] p-10 flex items-center justify-between">
            <div>
              <h3 className="text-[10px] font-black uppercase text-secondary mb-2 tracking-widest italic">Looking For</h3>
          ...
          <div className="space-y-6">
            <h3 className="text-[10px] font-black uppercase text-muted-foreground ml-6 tracking-widest">My Passions</h3>
            <div className="flex flex-wrap gap-3">
              {displayUser.interests?.map((tag: string) => (
                <span key={tag} className="px-8 py-4 bg-muted/40 border border-border text-[11px] font-black rounded-full uppercase hover:border-primary hover:bg-primary/5 transition-all">
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div className="flex gap-12 border-b border-border pt-6">
            {['stories', 'reviews'].map((tab) => (
              <button
                ...
                className={`pb-6 text-[11px] font-black uppercase tracking-[0.2em] italic flex items-center gap-2 transition-all ${activeTab === tab ? 'border-b-4 border-primary text-primary' : 'text-muted-foreground opacity-50 hover:opacity-100'}`}
              >
                {tab}
              </button>
            ))}
          ...
          {activeTab === 'stories' ? (
            <div className="py-24 text-center bg-card rounded-[4rem] border-2 border-dashed border-border group hover:border-primary/40 transition-all">
              <h3 className="text-2xl font-black italic uppercase tracking-tighter opacity-30">No Memories Posted</h3>
              {isOwnProfile && <button onClick={() => setIsStoryModalOpen(true)} className="mt-8 px-10 py-4 bg-primary text-white rounded-2xl font-black text-[11px] uppercase shadow-xl hover:scale-105 transition-all">Add First Story</button>}
            </div>
          ) : (
      ...
      {isMessageModalOpen && <MessageModal userName={displayUser.name} onClose={() => setIsMessageModalOpen(false)} />}
    </div>
  );
}

// ব্যাজ এবং ইনফো কম্পোনেন্টগুলো (সেম থাকবে)
function LifestyleBadge({ icon, label, value }: { icon: any, label: string, value?: string }) {
  return (
    <div className="flex items-center gap-3 p-5 bg-muted/30 rounded-[2rem] border border-transparent hover:border-primary/20 transition-all group">
      <div className="text-primary group-hover:scale-110 transition-transform">{icon}</div>
      <div>
        <p className="text-[9px] font-bold uppercase opacity-40 leading-none mb-1">{label}</p>
        <p className="text-[11px] font-black uppercase italic truncate max-w-[90px]">{value || "—"}</p>
      </div>
    </div>
  );
}
